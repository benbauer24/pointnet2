...
import tf_slim as slim                    #Slim module added using pip install --upgrade tf_slim
...
def _variable_on_cpu(name, shape, initializer, use_fp16=False):
  ...
  with tf.device("/cpu:0"):
    ...
    var = tf.compat.v1.get_variable(name, shape, initializer=initializer, dtype=dtype)   #replaced tf by tf.compat.v1
  return var
...
def _variable_with_weight_decay(name, shape, stddev, wd, use_xavier=True):
  ...
  """
  if use_xavier:
    initializer = slim.xavier_initializer()                    #replaced tf.contrib.layers by slim
  else:
    initializer = tf.truncated_normal_initializer(stddev=stddev)            #replaced tf by tf.compat.v1
...
def conv2d(inputs,
  ...
  with tf.compat.v1.variable_scope(scope) as sc:        #replaced tf by tf.compat.v1
      ...
      assert(data_format=='NHWC' or data_format=='NCHW')
      if data_format == 'NHWC':
        num_in_channels = inputs.get_shape()[-1]                #all .value removed
      elif data_format=='NCHW':
        num_in_channels = inputs.get_shape()[1]                 #all .value removed
...
def conv3d(inputs,
  ...
  Returns:
    Variable tensor
  """
  with tf.compat.v1.variable_scope(scope) as sc:        # all tf.variable_scope replaced by tf.compat.v1.variable_scope
